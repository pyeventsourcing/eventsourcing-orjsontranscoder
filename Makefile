...
COMPOSE_PROJECT_NAME ?= eventsourcing_orjsontranscoder
...
.PHONY: test
test:
	$(POETRY) run python -m pytest -rPv $(opts) $(call tests,.)
...
build-inplace:
	...
	$(POETRY) run cythonize -i  tests/_orjson_transcodings.pyx
